...
//nolint:gocognit,gocyclo,gocritic,goconst,dupl // Header builder is verbose and intentionally explicit for Docs API requests.
func (doc *FRD) ReplaceHeader(fileID, baseURL string, isDraft bool, provider workspace.Provider) error {
	...
	// PRFAQ cell.
	// TODO: We only show a link with the text "PRFAQ" now if a PRFAQ is defined,
	// but should show the document number instead.
	prfaqCellVal := ""
	if doc.PRFAQ != "" {
		prfaqCellVal = "PRFAQ"
	}
	cellReqs, cellLength = createTextCellRequests(
		"PRFAQ", prfaqCellVal, int64(pos))
	reqs = append(reqs, cellReqs...)
	if doc.PRFAQ != "" {
		reqs = append(reqs,
			[]*docs.Request{
				// Add link to PRFAQ.
				{
					UpdateTextStyle: &docs.UpdateTextStyleRequest{
						Fields: "link",
						Range: &docs.Range{
							StartIndex: int64(pos + 7),
							EndIndex:   int64(pos + 10),
						},
						TextStyle: &docs.TextStyle{
							Link: &docs.Link{
								Url: doc.PRFAQ,
	...
	// PRD cell.
	// TODO: We only show a link with the text "PRD" now if a PRD is defined, but
