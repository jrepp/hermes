...
# MEMO-126: Multi-Provider Auth Architecture (Diagrams)

> Demoted from ADR-084. This memo holds the reference diagrams for the multi-provider auth flows; the binding architectural decisions live in [ADR-076](../adr/adr-076-multi-provider-auth-architecture.md), [ADR-077](../adr/adr-077-auth-provider-selection.md), and [ADR-078](../adr/adr-078-dex-authentication-implementation.md). Diagrams alone are not a decision.
