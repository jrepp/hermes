...
- MEMO-061 — `ember-animated` passthrough stub components (sibling Ember-ecosystem migration note).

## History

- 2025-10-08: Originally written as ADR 029 mandating `ember-power-select ^8.11.0` + `ember-concurrency ^2.3.7`.
- 2026-04-26: Demoted to MEMO-064. ADRs should not pin specific package versions; that contract belongs in `package.json`. The dependency situation in `web/package.json` may have moved on; treat the live manifest as ground truth.