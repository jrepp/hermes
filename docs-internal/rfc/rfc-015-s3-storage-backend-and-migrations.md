...
### Existing Foundation (RFC-010)

Hermes already has excellent multi-backend support via RFC-010:
...
**Multi-Backend Document Example** (from RFC-010):
...
**Event-Driven Architecture using Kafka/Redpanda + Outbox Pattern (RFC-008, RFC-013)**
...
│                                                                       │
│ ┌─────────────────────────────────────────────────────────────────┐ │
│ │ Workspace Provider Interface (RFC-010)                           │ │
...
└────────────┬────────────────────────────────────────────────────────┘
    ...
    │ 6. Emit revision event → document_revision_outbox (RFC-014)    │
...
│ │ - Per-document state │ │ (NEW - RFC-008)      │                   │
...
#### Migration Outbox (Transactional Outbox Pattern - RFC-008)
...
func (m *Manager) migrateDocument(
    ...
    // Emit event to outbox for indexing (RFC-014)
    if !job.DryRun {
