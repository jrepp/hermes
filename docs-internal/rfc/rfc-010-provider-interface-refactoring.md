...
This RFC proposes a refactoring of the Hermes provider architecture to support multi-backend document tracking through focused provider interfaces and Hermes-native types. This is the foundation for enabling API providers (RFC-011) and authentication delegation (RFC-012).
...
**Related RFCs**:
- **RFC-011**: API Provider and Remote Delegation (implementation patterns)
- **RFC-012**: Authentication and Bearer Token Management (auth strategy)

## Context

### Current Document Model (RFC-021 Foundation)
...
- Local: `local:{relative/path/to/file.md}`
  - Example: `local:docs/rfc/RFC-010.md`
- GitHub: `github:{owner}/{repo}/{path}@{ref}`
  - Example: `github:hashicorp/hermes/docs/RFC-010.md@main`
...
doc, err := manager.GetDocument(ctx, "local:docs/rfc-010.md")
...
**Pattern 3: Full Delegation** (API Provider - see RFC-011):
...
- 🔵 **Delegated**: Delegated to remote Hermes API (see RFC-011)
...
- **RFC-021**: Document Identification System (UUID + ProviderID)
- **RFC-011**: API Provider and Remote Delegation (implementation patterns)
- **RFC-012**: Authentication and Bearer Token Management (auth strategy)
- **RFC-017**: Document Revisions and Migration Tracking
...
1. Review and approve RFC-010
2. Review RFC-011 (API Provider implementation patterns)
3. Review RFC-012 (Authentication strategy)
4. Begin Phase 1 implementation (type definitions)

---

**Status**: Proposed
**Implementation**: See RFC-011 for API provider patterns and RFC-012 for authentication strategy