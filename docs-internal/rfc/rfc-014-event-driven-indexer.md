...
Refactor the document indexer to use an event-driven architecture that combines the transactional outbox pattern (RFC-005/080) with Redpanda messaging (RFC-013). Document revisions trigger pipeline executions based on configurable rulesets, enabling search indexing, embedding generation, and LLM-powered summarization.
