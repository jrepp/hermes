...
# ADR-081: V2 API Provider Abstraction Pattern
...
See [RFC-094](../rfc/rfc-094-api-refactoring-and-testing-strategy.md) for the full migration narrative, integration-test framework, V1-vs-V2 analysis, and code metrics.
...
- Migration cost was substantial (~46–57 hours for the original refactor; see RFC-094).
...
- [RFC-094: API Refactoring and Testing Strategy](../rfc/rfc-094-api-refactoring-and-testing-strategy.md) — full narrative, metrics, migration history.
- [ADR-073: Provider Abstraction Architecture](adr-073-provider-abstraction-architecture.md) — the broader Strategy + Adapter + Factory + DI pattern.
- [ADR-075: Meilisearch as Local Search Solution](adr-075-meilisearch-as-local-search-solution.md) — concrete `search.Provider` implementation.
- [ADR-080: Search and Auth Refactoring](adr-080-search-and-auth-refactoring.md) — backend-mediated search pattern.
