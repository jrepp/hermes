...
# ADR-019: Split Server and Migrate Binaries (Pure-Go Server)
...
- **Keep migrations in the server binary but use `replace` directives or import aliases** — proven to fail (see MEMO-063).

## References

- [MEMO-063: SQLite Driver Conflict Investigation](../memo/memo-063-sqlite-driver-conflict-investigation.md) — full root-cause analysis and seven attempted fixes.
- [ADR-020: Core+Deltas Migrations and Stateless Indexer](adr-020-dual-database-support-stateless-indexer.md) — migration file layout that `cmd/hermes-migrate` runs.
