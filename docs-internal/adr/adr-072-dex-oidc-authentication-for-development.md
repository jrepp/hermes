...
related: ['ADR-070', 'ADR-076', 'ADR-078']
created: 2026-04-24
...
Run Dex as a containerized OIDC provider in the testing stack with a static-password connector. Hermes' Dex auth adapter (`pkg/auth/adapters/dex/`) implements the same `auth.Provider` interface (ADR-073, ADR-076) as the Google and Okta adapters; switching is purely a configuration change.
...
- ADR-076 (multi-provider auth), ADR-078 (Dex implementation), ADR-070 (testing stack)
