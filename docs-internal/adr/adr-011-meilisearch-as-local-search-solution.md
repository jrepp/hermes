...
> Local development, CI, and acceptance testing use **Meilisearch `v1.11`** (`getmeili/meilisearch`) as the search backend. Production uses **Algolia**. Both sit behind `search.Provider` (ADR-009), and selection is `providers.search = "meilisearch" | "algolia"` in HCL. The frontend never talks to either directly — all search goes through the backend `/1/indexes/*` API (ADR-016).
...
- **Deployment:** containerized in `testing/docker-compose.yml` on port `7701` (ADR-006).
...
- Frontend stays credential-free because it only talks to the backend (ADR-016).
...
- ADR-006 (testing stack), ADR-009 (provider abstraction), ADR-016 (backend-mediated search)