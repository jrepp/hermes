...
Hermes runs in environments with very different identity stories: hashicorp.com Google Workspace in production, Okta for enterprise customers, and Dex with static passwords for local dev and CI (ADR-008). Auth flows split into two shapes — client-side OAuth (Google popup → access token) and server-side OIDC (redirect → backend code exchange → session cookie) — and the frontend has to handle both without a build-time choice.
...
- Adding a new OIDC IdP is an `auth.Provider` adapter (ADR-009) plus a config switch; no frontend code change.
...
- ADR-009 (provider abstraction), ADR-013 (CLI/env override), ADR-014 (Dex implementation), MEMO-062 (auth flow diagrams)