...
# ADR-085: Core+Deltas Migrations and Stateless Indexer
...
See [RFC-096](../rfc/rfc-096-database-deltas-and-stateless-indexer.md) for the full migration architecture, indexer-registration model, delta-maintenance metrics, file inventory, and roadmap.
...
5. **Future: per-binary `go.mod`.** `cmd/hermes`, `cmd/hermes-indexer`, and `cmd/hermes-migrate` are designed to be separable modules so the indexer can ship without database drivers and the migrate binary can own SQLite (see [ADR-083](adr-083-split-server-and-migrate-binaries.md)).
...
- Foundation for local-developer mode (RFC-001) and remote indexers (RFC-085).
...
- [RFC-096: Database Deltas and Stateless Indexer](../rfc/rfc-096-database-deltas-and-stateless-indexer.md) — full architecture, migration files, models, metrics, roadmap.
- [ADR-083: Split Server and Migrate Binaries](adr-083-split-server-and-migrate-binaries.md) — pure-Go server, dedicated migrate binary owning SQLite.
- [ADR-081: V2 API Provider Abstraction Pattern](adr-081-api-refactoring-and-testing-strategy.md) — indexer submits through V2 endpoints.
